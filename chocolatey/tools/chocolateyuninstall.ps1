...
$key = Get-UninstallRegistryKey -SoftwareName 'SoundGeek*'
...
elseif ($key.Count -eq 0) {
  Write-Warning 'SoundGeek is not installed, or was installed outside of Chocolatey. Nothing to do.'
}
else {
  Write-Warning "$($key.Count) matches found for SoundGeek. Remove it manually rather than guessing:"
